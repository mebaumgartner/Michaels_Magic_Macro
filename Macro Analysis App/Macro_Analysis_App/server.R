...
server <- function(input, output) {
  ...
  values$cols2 <- c()
  output$plotso <- renderUI({img(src = "Logo.png", height="65%", width="65%")}) #This is the logo used in place of the plot
  ...
  output$analysisType = renderUI({
    ...
    selectInput("sepTimePoints", "Specify Analysis Type:", c("Default", "Time-lapse analysis", "Z-axis analysis", "Complete single cell analysis", "Complete single cell analysis - timelapse"))
  })
  ...
  output$effectSize = renderUI({
    if (isTRUE((input$plotType == "Scatter Plot") | (input$plotType == "Scatter Plot w/ Regression Line") )){
      ...
      effOptions <- c("Cliff's Delta", "Cohen's D - Pooled","Cohen's D - Not Pooled", "Hedges' G - Pooled", "Hedges' G - Not Pooled") 
    }
  ...
  output$exclusioninputreg4 = renderUI({
    validate(
      need((input$linkFunction == "Logistic") && (((input$sepTimePoints == "Complete single cell analysis") | (input$sepTimePoints == "Complete single cell analysis - timelapse"))), "")
    )
  ...
  output$regressionSelectExclude = renderUI({
    validate(
      need((input$linkFunction == "Logistic") && (((input$sepTimePoints == "Complete single cell analysis") | (input$sepTimePoints == "Complete single cell analysis - timelapse")))&& (input$excludecheckreg2==TRUE), "")
    )
  ...
  output$exclusioninputreg2 = renderUI({
    validate(
      need((input$linkFunction == "Logistic") && (((input$sepTimePoints == "Complete single cell analysis") | (input$sepTimePoints == "Complete single cell analysis - timelapse"))) && (input$excludecheckreg==FALSE)&& (input$excludecheckreg2==TRUE), "")
    )
  ...
  output$factorCheckboxes = renderUI({
    ...
    checked <- grep("ConditionAndExperiment|ROI.Volume|Percent.Clone.Coverage.of.ROI", factorChoices)
  ...
  output$secondCsv = renderUI({
    validate(
      need(((input$sepTimePoints == "Complete single cell analysis") | (input$sepTimePoints == "Complete single cell analysis - timelapse")), "")
    )
  ...
  observeEvent(input$saveBtn, {
    ...
    wda <- data.frame(Image.Name = c(1,2,3,4,5)) #This is a placeholder for our first dataframe, wda for Experiment-data-assign
    ...
    tryCatch(
      
      {
        ...
        FileImage <- data.frame(File = wda$File, Image.Name = wda$Image.Name)
        
        FileImage[
          with(FileImage, order(File, Image.Name)),
          ]
    ...
    tryCatch(
      {
        if (randoTable == TRUE){
          ...
          ID <- 1:nrow(wda)
          Experiment <- rep("Specify experiment/grouping", nrow(wda))
  ...
  analysis <- observeEvent(input$go,{
    ...
    output$plotso <- renderUI({img(src = "Logo.png", height="65%", width="65%")})
    ...
    tryCatch(
      {
        ...
        for (i in colnames(wwda2)){
          ...
          if(grepl("Border.Foci.Count", i, fixed = TRUE)){
            TrueCasCounts <- 1
            genotypeNames <- c(genotypeNames, gsub("Border.Foci.Count.", "", i ))
          }
        }
          ...
          #Restore Foci counts
          {
            ...
            if (TrueCasCounts == 1 && TrueNonCasCounts == 1){
              ...
              for (i in genotypeNames){
                ...
                centerCasCells <- aggregate(genotypeVals$Center.Foci.Count, list(genotypeVals$ConditionAndExperiment), sum)
  ...
  analysis <- observeEvent(input$go, {
    ...
    tryCatch({
      ...
      wwda2$ConditionAndExperiment <- paste(wwda2$Condition, wwda2$Experiment, sep=", ")
    ...
    tryCatch(
      {
        ...
        ccta2 <- cbind( CloneZName = paste(ccta$Image.Name ,ccta$Experiment, ccta$Clone.ID , ccta$Z.level, ccta$TimePoint, sep=", ") , ccta )
        ...
        tryCatch(
          ...
          {
            cctz <- cbind( CloneZID = paste(ccta$Image.Name ,ccta$Experiment, ccta$Clone.ID) , ccta )
        ...
        ccta2sum <- data.frame(
          ...
          Condition =  as.character(tapply(as.character(ccta2$Condition) , ccta2$CloneZName , unique)),
          CloneZName = as.character(tapply(as.character(ccta2$CloneZName) , ccta2$CloneZName , unique)),
          Experiment =  as.character(tapply(as.character(ccta2$Experiment) , ccta2$CloneZName , unique)),
        ...
        tryCatch({
          ccta3 <- cbind(ccta3, Time.Point = tapply(ccta2$TimePoint, ccta2$CloneZName, unique))
        ...
        ccta3sum <- data.frame(
          ...
          Experiment =  tapply(as.character(ccta3$Experiment) , ccta3$CloneName , unique),
        ...
        tryCatch(
          # accounts for a change in format between older macro and newer macro
          {
            ccta3sumZ2 <- cbind(ccta3sum, Time.Point = tapply(as.character(ccta3$Time.Point) , ccta3$CloneName , unique))
          },
          # ... but if an error occurs, tell me what happened:
          error=function(error_message) {
            message("no timepoint")
        ...
        tryCatch(
          # This is what I want to do...
          {
            ...
            {  
              ...
              ccta3sumZ2 <- cbind(ccta3sumZ2, Clone.Volume.Normalized.to.ROI= clonepercent)
            }
          },
          # ... but if an error occurs, tell me what happened:
          error=function(error_message) {
        ...
        wwda2 <- ccta3sumZ2
        wwda2$ConditionAndExperiment <- paste(wwda2$Condition, wwda2$Experiment, wwda2$Genotype, sep=", ")
        ...
        output$contents <- renderTable(wwda2,  hover=TRUE, border=TRUE, spacing="s", digits=5, align ="l", width="auto")
        values$rawChoices <- colnames(wwda2)
        wwda2sub <- subset(wwda2, select = -c(CloneID, Condition, CloneName, Experiment, ConditionAndExperiment))
  ...
  analysis <- observeEvent(input$go, {
    ...
    tryCatch(
      {
        ...
        wwdaFull <- merge(x = wda, y = trad, by=c("File", "Image Name"))
        
        
        if (isFALSE("TimePoint" %in% names(wwdaFull))){
          
          wwdaFull$"Image Name" <- paste(wwdaFull$"File", wwdaFull$"Image Name", sep="")
        } else {
          if (isFALSE(input$sepTimePoints == "Time-lapse analysis"))
            wwdaFull$"Image Name" <- paste(wwdaFull$"Image Name", wwdaFull$"TimePoint", sep=", timepoint:")
        }
        ...
        while(chal <= count){
          ...
          wwda2 <- data.frame(
            ...
            Condition =  tapply(as.character(wwda$Condition) , wwda$Image.Name , unique),
            Experiment = tapply(as.character(wwda$Experiment) , wwda$Image.Name , unique),
          ...
          if("Cell.Count.in.Center" %in% colnames(wwda))
          {
            ...
            wwda2 <- cbind(wwda2, Total.Number.of.Cells = tapply(wwda$Cell.Count.in.Border , wwda$Image.Name , unique)+tapply(wwda$Cell.Count.in.Center , wwda$Image.Name , unique))
          ...
          if("Total.number.of.Foci" %in% colnames(wwda))
          {
            ...
            if ("Total.Number.of.Cells" %in% colnames(wwda2))
            {
              ...
              wwda2$Number.of.Clone.Cells.And.Foci <- wwda2$Total.Number.of.Cells + wwda2$Clone.Foci.Count
              wwda2$Number.of.Center.Cells.And.Foci <- wwda2$Number.of.Center.Cells + wwda2$Center.Foci.Count
              wwda2$Number.of.Border.Cells.And.Foci <- wwda2$Number.of.Border.Cells + wwda2$Border.Foci.Count
            }
          ...
          {
            wwda2 <- cbind(wwda2, Percent.Foci.Positive.Cells.in.Clones = wwda2$Clone.Foci.Count * 100/ wwda2$Number.of.Clone.Cells.And.Foci)
            wwda2 <- cbind(wwda2, Percent.Foci.Positive.Cells.in.Border = wwda2$Border.Foci.Count * 100/ wwda2$Number.of.Border.Cells.And.Foci)
            wwda2 <- cbind(wwda2, Percent.Foci.Positive.Cells.in.Center = wwda2$Center.Foci.Count * 100/ wwda2$Number.of.Center.Cells.And.Foci)
          ...
          tryCatch(
            # This is we run the fisher test for individual wing discs
            {
              ...
              if(input$runFishers2 == TRUE){
                ...
                fishResult2I <- data.frame(Image.Name = wwda2$Image.Name, ConditionAndExperiment = wwda2$ConditionAndExperiment, fishResult, conf.interval.from = fishConfidence, conf.interval.to = fishConfidenceUB, p.adjusted = as.numeric(pAdj), p.adjustment.method = input$pAdj, odds.ratio=fishOddsRatio)
                output$siggy <- renderTable(fishResult2I, hover=TRUE, border=TRUE, spacing="s", digits=5, align ="l")
                fishResult2 <- data.frame(Image.Name = wwda2$Image.Name, ConditionAndExperiment = wwda2$ConditionAndExperiment, fishResultNum)
                Experimentname <- unique(fishResult2$ConditionAndExperiment)
          ...
          wwda2sub <- subset(values$wwda2, select = -c(File, Image.Name, Condition, Experiment, ConditionAndExperiment))
          values$choices <- colnames(wwda2sub)
    ...
    tryCatch({
      ...
      validate(
        need(((input$sepTimePoints == "Complete single cell analysis") | (input$sepTimePoints == "Complete single cell analysis - timelapse")), "Select complete single cell analysis")
      ...
      singleCellDf$Image.Name <- as.character(singleCellDf$Image.Name)
      if (input$sepTimePoints == "Complete single cell analysis - timelapse"){
        ##place
        singleCellDf$Image.Name <- paste(singleCellDf$Image.Name, singleCellDf$TimePoint, sep=", timepoint:")
      } else {
      ...
      wwda2$Image.Name <- gsub( ', timepoint:.*', "", wwda2$Image.Name)
    ...
    tryCatch(
      
      {
        ...
        tryCatch(
          ...
          {
            wwda2sub <- subset(wwda2, select = -c(File, Image.Name, Condition, Experiment, ConditionAndExperiment))
            values$choices <- colnames(wwda2sub)
          },
    ...
    tryCatch({
      ...
      if (input$linkFunction == "Logistic"  ) {
        ...
        tryCatch({
          ...
          padjreg <- p.adjust(pvalsReg, method=input$regAdjust ,n=length(pvalsReg))
          ...
          colnames(logOddsVec) <- c(tipo, "Odds Ratio", "Conf. Int. Lower", "Conf. Int. Upper")
        ...
        tryCatch({
          ...
          colnames(rexp.est) <- c(paste("Adjusted p-value(", input$regAdjust, ")", sep=""), "Incident Rate Ratio", "IRR Robust Std. Err", "IRR Conf. Int. Lower", "IRR Conf. Int. Upper")
      ...
      output$RegEffectsPlot <- renderPlot( plot(predictorEffects( regression, formulaP), main="", lines=list(multiline=TRUE, z.var="Condition"),type="response"))
  ...
  analysis <- observeEvent(input$columnsReg, {
    ...
    output$RegEffectsPlot <- renderPlot( plot(predictorEffects( regression, formulaP), main="",  lines=list(multipline=TRUE), type="response"))
    values$formulaP <- formulaP
  ...
  analysis <- observeEvent(input$genPlot, { 
    ...
    output$leveneHeader <- renderText("")
    output$plotso <- renderUI({img(src = "Logo.png", height="65%", width="65%")})
    output$fishBC <- renderTable(data.frame(), hover=TRUE, border=TRUE, spacing="s", digits=5, align ="l")
    output$fishyPrint <- renderText("")
    output$fishyCounty <- renderTable(data.frame(), hover=TRUE, border=TRUE, spacing="s", digits=5, align ="l")
    output$fishTable<- renderTable(data.frame(), hover=TRUE, border=TRUE, spacing="s", digits=5, align ="l")
    print("x")
    ...
    if(input$orderWeek == TRUE){ values$wwda2 <- values$wwda2[order(values$wwda2$Experiment)] } 
    if(input$orderGene == TRUE){ values$wwda2 <- values$wwda2[order(values$wwda2$Condition)] }
    ...
    if ((input$plotType == "Paired Box Plot")|(input$plotType == "Scatter Plot") | (input$plotType == "Scatter Plot w/ Regression Line")| (input$plotType == "Line Plot") | (((input$plotType == "Histogram" ) | (input$plotType == "Histogram w/ Density Plot")) & isFALSE(input$plotterpaired=="N/A") ) ){
      ...
      plotformula2 <- paste(xValP, "ConditionAndExperiment", sep=" ~ ")
      ...
      # This is what I want to do...
      {
        ...
        if(input$plotType == "Paired Box Plot"){
          ...
          if(isTRUE(input$plotterpaired=="N/A")){
            ...
            p1<- ggplot( values$wwda2 , aes( x= as.numeric(values$plotData), color = Condition,  fill = Condition )) 
          } else { 
          ...
          dots <- input$dots + 2
          p1 <- ggplot(values$wwda2, aes(x=values$plotData, y=values$plotData2, color=values$wwda2$Condition, fill = values$wwda2$Condition, shape = values$wwda2$Condition)) + geom_point(size = dots)
          pairedvolo <- FALSE
          if((input$xValue == "") & (input$xDel == FALSE)){xtitle <- xValP }
          if(input$y == ""){ ytitle <- yValP }
          ...
          if(input$sepTimePoints== "Z-axis analysis"){
            test_data<- values$wwda2[,c("Image.Name", xValP, "ConditionAndExperiment", input$plotterpaired)]
            test_data$Image.Name <- sub(".tp.*", '', test_data$Image.Name)
            names(test_data)[names(test_data)==input$plotterpaired] <-"y.axis.variable"
            p1<- ggplot(test_data, aes(x=.data[[xValP]], y=y.axis.variable, color=ConditionAndExperiment, group=Image.Name)) + geom_line()
        ...
        #add theme
        p1<- p1 + ggtitle(toString(values$plotOutTitle)) +labs( x= xtitle, y = ytitle )+ theme(
          ...
          axis.text.y = element_text(angle = 45 , hjust = 1 , face = "bold" , size = values$axisFont),
            ...
            if ((input$plotType != "Scatter Plot")&(input$plotType != "Scatter Plot w/ Regression Line")& (input$plotType != "Line Plot")){
              ...
              if (pairedvolo == FALSE){
                ...
                if(input$refgroup == "All/Basemean"){
                  values$statistics <- compare_means(plotformula, data = values$wwda2, ref.group=".all.", method = values$method, p.adjust.method=input$pAdj,  group.by= "Experiment")
                  
                } else if (input$refgroup == "None"){
                  values$statistics <- compare_means(plotformula, data = values$wwda2, method = values$method, p.adjust.method=input$pAdj,  group.by= "Experiment")
                } else if (input$refgroup == "N/A"){
                  values$statistics <- compare_means( xValues ~ Identification, data = values$pairDataStat, method = values$method, group.by= "ConditionAndExperiment",  paired = TRUE)
                }else {
                  values$statistics <- compare_means(plotformula, data = values$wwda2, method = values$method, ref.group=input$refgroup, p.adjust.method=input$pAdj,  group.by= "Experiment")
                }
                ...
                if(input$refgroup != "None"){
                  values$statistics <- compare_means( xValues ~ Identification, data = values$pairDataStat, method = values$method, p.adjust.method=input$pAdj, group.by= "ConditionAndExperiment",  paired = TRUE)
                } else {
                  values$statistics <- compare_means( xValues ~ Identification, data = values$pairDataStat, method = values$method,  p.adjust.method=input$pAdj, group.by= "ConditionAndExperiment",  paired = TRUE)
                }
        ...
        tryCatch(
          # Try statement for effect size
          {
            ...
            if (input$plotType == "Paired Box Plot"){
              effectData <- values$wwda2[,c(xValP, yValP, "Condition", "Experiment", "ConditionAndExperiment")]
              effectData <- na.omit(effectData)
              
            } else {
              effectData <- values$wwda2[,c(plottitle, "Condition", "Experiment", "ConditionAndExperiment")]
              effectData <- na.omit(effectData)
            ...
            while (x < lengthotron ){
              ...
              if (input$plotType == "Paired Box Plot"){
                ExperimentData <- effectData %>% filter(ConditionAndExperiment == Experimentname)
                genes <- unique(ExperimentData$ConditionAndExperiment)
                pairing = TRUE
                
                
              } else {
                ExperimentData <- effectData %>% filter(Experiment == Experimentname)
              ...
              for (genename in genes){ 
                ...
                if(input$plotType == "Paired Box Plot"){
                  ...
                  ExperimentData <- effectData %>% filter(ConditionAndExperiment == genename)
                  ...
                  for (genename2 in setdiff(genes, priorGenes)) {
                    ...
                    if (genename != genename2){
                      ...
                      if (input$refgroup == "All/Basemean"){
                        ...
                        expData <- newData %>% filter((Condition == genename))
                        ...
                        newData <- ExperimentData[(ExperimentData$Condition == genename) |(ExperimentData$Condition == genename2), ]
                        
                        #Filter out dataset so it corresponds just to current experiments
                        
                        expData <- newData %>% filter((Condition == genename)|(Condition == genename2))
                      }
      ...
      if (input$statsType != "Hotellings T-Squared Test"){
        tryCatch(
          # Attempt to run correlations tests
          {
            ...
            #Get the names and identifiers of all our datasets and datapoints
            ConditionAndExperiments <- unique(values$wwda2$ConditionAndExperiment)
            ...
            while (x < lengthotron ){
              ...
              ExperimentData <- values$wwda2 %>% filter(ConditionAndExperiment == Experimentname)
              ...
              for (genename in genes){
                ...
                for (genename2 in setdiff(genes, priorGenes)) {
                  if (genename != genename2){
                    ...
                    corry <- cor.test(ExperimentData[,xValP], y = ExperimentData[,yValP], use="pairwise.complete.obs", method = values$corrMethod)
                    ...
                    corrConditionAndExperiment <- rbind(corrConditionAndExperiment, as.character(Experimentname))
            ...
            corrResult2 = data.frame(Group = corrConditionAndExperiment, X = corrConditions, Y = corrConditions2, corrResult, p.adjusted = as.numeric(pAdj), p.adjustment.method = input$pAdj)
        ...
        tryCatch(
          # Run Hotellings t2
          {
            ...
            newData <- cbind(values$wwda2["ConditionAndExperiment"], newData)
            ...
            for (Experimentname in ConditionAndExperiments){
              ...
              for (Experimentname2 in setdiff(ConditionAndExperiments, priorGenes)){
                ...
                if (Experimentname != Experimentname2){
                  ...
                  newData2 <- newData %>% filter((ConditionAndExperiment == Experimentname) | (ConditionAndExperiment == Experimentname2))
                  output$contents <- renderTable(newData2,  hover=TRUE, border=TRUE, spacing="s", digits=5, align ="l", width="auto")
                  
                  newData.vars <-  newData2 %>% dplyr::select(-one_of("ConditionAndExperiment"))
                  ...
                  hotellings <- HotellingsT2(filter(newData2, ConditionAndExperiment == Experimentname )[,2:3],
                                             filter(newData2, ConditionAndExperiment == Experimentname2 )[,2:3])
                  ...
                  corrConditionAndExperiment <- rbind(corrConditionAndExperiment, as.character(Experimentname))
                  corrConditionAndExperiment2 <- rbind(corrConditionAndExperiment2, as.character(Experimentname2))
            ...
            pAdj <- p.adjust(pVals, method = input$pAdj, n = length(pVals))
            corrResult2 = data.frame(Group1 = corrConditionAndExperiment, Group2 = corrConditionAndExperiment2, X = corrConditions, Y = corrConditions2, corrResult, p.adjusted = as.numeric(pAdj), p.adjustment.method = input$pAdj)
    ...
    if((input$plotType != "Scatter Plot") & (input$plotType != "Scatter Plot w/ Regression Line")){
      ...
      tryCatch(
        # Try statement for equivalence of variance test
        {
          ...
          #Get the names and identifiers of all our datasets and datapoints
          flignerData <- values$wwda2[,c(plottitle, "Condition", "Experiment", "ConditionAndExperiment")]
          ...
          while (x < lengthotron ){
            ...
            if (input$plotType == "Paired Box Plot"){
              ...
              ExperimentData <- flignerData %>% filter(Experiment == Experimentname)
              genes <- unique(ExperimentData$Condition)
            }
              ...
              for (genename2 in setdiff(genes, priorGenes)) {
                ...
                if (genename != genename2){
                  ...
                  if (input$plotType == "Paired Box Plot"){
                    ...
                    flignerVector1 <- c(ExperimentData[, genename], ExperimentData[, genename2])
                    filler <- length(flignerVector1) / 2
                    flignerVector2 <- c(rep(genename, filler), rep(genename2, filler))
                    
                  } else {
                    geneData <- ExperimentData %>% filter(Condition %in% c(genename, genename2))
                    flignerVector1 <- geneData[,plottitle]
                    flignerVector2 <- as.factor(geneData[,"Condition"])
                  }
    ...
    if( input$statsType != "Hotellings T-Squared Test"){
      tryCatch(
        # Try statement for shapiro-wilks test
        {
          ...
          shapiroRes <- tapply(values$wwda2[,plottitle] , values$wwda2$ConditionAndExperiment , shapiro.test)
          shapiroP <- c()
          ...
          if ((input$plotType == "Paired Box Plot") | (input$plotType == "Scatter Plot") | (input$plotType == "Scatter Plot w/ Regression Line")| (input$plotType == "Line Plot")){
            ...
            shapiroRes <- tapply(values$wwda2[,input$plotterpaired] , values$wwda2$ConditionAndExperiment , shapiro.test)
            shapiroP <- c()
