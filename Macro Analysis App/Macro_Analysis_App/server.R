...
server <- function(input, output) {
  ...
  analysis <- observeEvent(input$go, {
    ...
    #   {
      ...
      colnames(trad) <- c("File", "Image Name", "week", "Gene.Name")
                ...
                if("Total.Caspase.Cells" %in% colnames(wwda))
                 {
                    ...
                    if ("Total.Number.of.Cells" %in% colnames(wwda2))
                    {
                      ...
                      wwda2$Total.Number.of.Cells.Clones <- wwda2$Total.Number.of.Cells + wwda2$Clone.Caspase.Cell.Count
                      wwda2$Number.of.Center.Cells <- wwda2$Number.of.Center.Cells + wwda2$Center.Caspase.Cell.Count
                      wwda2$Number.of.Border.Cells <- wwda2$Number.of.Border.Cells + wwda2$Border.Caspase.Cell.Count
                    }
    ...
    regressionTesting <- observeEvent(input$goReg, {
     ...
     tryCatch({
            ...
            output$RegEffectsPlot <- renderPlot( plot(predictorEffects( regression, formulaP), main="", lines=list(multiline=TRUE, z.var="GeneName"),type="response"))
         ...
         tryCatch(
            # This is what I want to do...
            {
               ...
               #
                    ...
                    dots <- input$dots + 2
                    p1 <- ggplot(values$wwda2, aes(x=values$plotData, y=values$plotData2, color=values$wwda2$GeneName, fill = values$wwda2$GeneName, shape = values$wwda2$GeneName)) + geom_point(size = dots)
                    pairedvolo <- FALSE
                    ...
                    if ((input$plotType != "Scatter Plot")&(input$plotType != "Scatter Plot w/ Regression Line")& (input$plotType != "Line Plot")){
                      ...
                      if (pairedvolo == FALSE){
                        ...
                        if(input$refgroup == "All/Basemean"){
                          ...
                          values$statistics <- compare_means( xValues ~ Identification, data = values$pairDataStat, method = values$method, group.by= "GeneWeek",  paired = TRUE)
                        }else {
