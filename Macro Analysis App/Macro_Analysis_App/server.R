...
server <- function(input, output) {
    ...
    values$control2 <- c("None", "All/Basemean")
    values$rawChoices <- c("First, analyze your data!")
    values$genAssign <-data.frame(c("First, upload your data"), c("Then, click 'Assign Genotypes'"))
    ...
    analysis <- observeEvent(input$go, {
       ...
       tryCatch(
        ...
        while(chal <= count){
            ...
            wwda2 <- cbind(wwda2, Estimated.Total.Cell.Count = round(exp(6.44112 + sqrt(wwda2$Total.Caspase.Cell.Count)*-1.892*10^(-2) + (wwda2$Caspase.Cell.Density.in.Border^(1/3))*-0.15175 + wwda2$Pouch.Volume*5.014*10^(-6) +  wwda2$Number.of.Z.Slices* -5.021*10^(-2)  + wwda2$Percent.Clone.Coverage.of.Pouch*3.023*10^(-2) + wwda2$Border.Coverage.of.Clones*-3.227*10^(-3) + wwda2$Caspase.Cell.Density.in.Center * 22.88436 )))
    ...
    regressionTesting <- observeEvent(input$goReg, {
     ...
     tryCatch({
            ...
            # Unable to order regression dataset
            error=function(error_message) {
              message("Unable to order regression dataset")
              return(NA)
            }
            )
            ...
            if (input$linkFunction == "Logistic") {
                ...
                } else { regression <- glm(logRegData[[nullVal]]~. , family = binomial(link="logit"), data=logRegDataSub, na.action = na.omit) }
                tryCatch({
                ...
                error=function(error_message) {
                  ...
                  tryCatch({
                    ...
                    predicted.data <- data.frame(
                      ...
                      } else { probability.observed = logRegData[[posVal]]}
                      )
                    ...
                    # Unable to generate regression plot
                    error=function(error_message) {
                        message("Unable to generate regression plot")
                        return(NA)
                    }
                    )
                ...
                regression <- glm(logRegData[[nullVal]]~., family="poisson", data=logRegDataSub, na.action = na.omit )
