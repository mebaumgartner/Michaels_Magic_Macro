...
def Open_GUI():
	...
	while gdLoop == 1:
		...
		options = ["Select option", "Save current settings", "Restore original default settings"]
		segChoices = []
		preProcess = ["None"] 
		...
		if gd.wasOKed():
			...
			elif saveLoad == "Restore original default settings":	
				gdLoop = 1
				#If the user chooses to restore the original defaults, we replace the modified config files with an archival file
			
	
				sourcePath = os.path.join(dir2, "configBasicArchive.py")
				destPath = os.path.join(dir2, "configBasic.py")
				copy(sourcePath, destPath)

				sourcePath = os.path.join(dir2, "configCellTrackArchive.py")
				destPath = os.path.join(dir2, "configCellTrack.py")
				copy(sourcePath, destPath)

				sourcePath = os.path.join(dir2, "configCloneSegArchive.py")
				destPath = os.path.join(dir2, "configCloneSeg.py")
				copy(sourcePath, destPath)

				sourcePath = os.path.join(dir2, "configDeathSegArchive.py")
				destPath = os.path.join(dir2, "configDeathSeg.py")
				copy(sourcePath, destPath)

				sourcePath = os.path.join(dir2, "configDeathTrackArchive.py")
				destPath = os.path.join(dir2, "configDeathTrack.py")
				copy(sourcePath, destPath)

				sourcePath = os.path.join(dir2, "configSaveArchive.py")
