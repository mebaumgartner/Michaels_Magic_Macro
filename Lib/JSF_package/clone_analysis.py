...
def clone_analysis(IDs, Title, pouch, excludinator, stackno, iHeight, rm, sliceROIs, borderArray, IDs3, cellROIs, numGenotypes, timepoint, colorArray, seedIDclone, segmentedIDs, genotypeNames, roiMask, iWidth):
	...
	#Launch our custom analysis options, if prompted by user
		...
		while activeGenotype <= numGenotypes:
			...
			resultantDup.getProcessor().setThreshold(activeGenotype, activeGenotype, 0)
			...
			if activeGenotype != 0:
				...
				resultantDupDup = resultantDup.duplicate()
				...
				genotypesImpArray.extend([resultantDup])
			else:
